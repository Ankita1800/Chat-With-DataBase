...
export default function Home() {
  ...
  const [infoModalData, setInfoModalData] = useState<{ title: string; message: string; type: "error" | "info" }>({ title: "", message: "", type: "info" });
  const [showConfirmModal, setShowConfirmModal] = useState(false);
  const [confirmModalData, setConfirmModalData] = useState<{ title: string; message: string; onConfirm: () => void }>({ title: "", message: "", onConfirm: () => {} });

  const fileInputRef = useRef<HTMLInputElement>(null);

  /**
   * Show info/error modal (replaces browser alert)
   */
  const showInfo = (title: string, message: string, type: "error" | "info" = "info") => {
  ...
  const handleFileUpload = async (forceUpload: boolean = false) => {
    if (!file) return;
    if (!user) {
      showInfo("Sign In Required", "Please sign in to upload files", "info");
      return;
    }
    ...
    try {
      ...
      showInfo("Upload Error", `Error uploading file: ${error instanceof Error ? error.message : "Unknown error"}`, "error");
      setIsUploading(false);
      setUploadProgress(0);
    }
  };
  ...
  const handleReuseDataset = async () => {
    ...
    try {
      ...
      showInfo("Reuse Error", `Error reusing dataset: ${error instanceof Error ? error.message : "Unknown error"}`, "error");
      setIsUploading(false);
      setUploadProgress(0);
  ...
  const handleDrop = (e: React.DragEvent) => {
    ...
    if (droppedFile && droppedFile.name.endsWith(".csv")) {
      ...
      showInfo("Invalid File Type", "Please upload a CSV file", "info");
    }
  };
  ...
  const askAI = async () => {
    ...
    if (!user) {
      showInfo("Sign In Required", "Please sign in to ask questions", "info");
      return;
    }
    if (!selectedDataset) {
      showInfo("No Dataset", "Please upload a dataset first", "info");
      return;
    }
    ...
    <div className="min-h-screen flex flex-col" style={{ backgroundColor: '#FDFBD4' }}>
      ...
      <div className="flex flex-1 relative">
        ...
        <aside
          ...
          } w-80 sm:w-96 lg:w-80 xl:w-96 fixed lg:fixed inset-y-0 left-0 z-30 transition-all duration-300 flex flex-col overflow-hidden`}
